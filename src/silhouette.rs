...
impl Silhouette {
    ...
    pub fn target_coverage(
        ...
        // Explicit alpha already supplies the silhouette. Internal color ridges
        // are not closed boundaries and must not carve transparent holes into
        // a boot, limb, or other opaque part of that source silhouette.
        ...
        let mut interior = vec![false; hard.len()];
        for y in 0..h {
            for x in 0..w {
                let i = y * w + x;
                if i.is_multiple_of(4096) {
                ...
                interior[i] = eroded && retained_ink[i] == 0.;
                trusted[i] = core.as_raw()[i] != 0 || interior[i];
            }
        }
        // Thin components with no eroded fill interior cannot safely be
        // bounded by shortened contour cores. Preserve their source support,
        // including terminal rows even when part of the component has a core.
